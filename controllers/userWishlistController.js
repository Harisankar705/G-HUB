...
userWishlistController.displayWishlist = async (req, res) => {
  try {
    ...
    let wishlist = await wishlistSchema.findOne({ userId: userId }).populate({
...
userWishlistController.addToWishlist = async (req, res) => {
  try {
    ...
    if (!product.category.isPublished) {
      return res.json({
        status: "error",
        message: "The category of the product is not available",
      });
    }
    ...
    if (wishlist.products.length > 0) {
      productAlreadyExists = wishlist.products.find(
...
userWishlistController.removeProduct = async (req, res) => {
  try {
    ...
    const wishlist = await wishlistSchema
      ...
      .populate("products.productId");
    const productIndexToRemove = wishlist.products.findIndex(
      (product) => product._id.toString() === productToRemove.toString()
    ); //removing product based on the object id
