...
productController.handleProduct = async (req, res) => {
  try {
    ...
    const additionalImagePaths = req.files["additionalImage"].map(
      (file) => file.path
    );
...
productController.editProduct = async (req, res) => {
  try {
    ...
    if (name.length > 255) {
      return res
        .status(400)
        .json("Product name should be less than 255 characters.");
    }
    ...
    if (description.length > 1000) {
      return res
        .status(400)
        .json("Product description should be less than 1000 characters.");
    }
...
productController.toggleManage = async (req, res) => {
  try {
    ...
    if (product) {
      ...
      const allProductsUnpublished =
        (await productSchema.find({ isPublished: true }).countDocuments()) ===
        0;
