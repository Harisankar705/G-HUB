...
loginController.showLandingPage = async (req, res) => {
  try {
    const user = await User.find();
    const product = await Product.find({ stock: 9 }).limit(1);
    const producttwo = await Product.find().sort({ stock: 1 }).limit(1);
    const productThree = await Product.find().sort({ stock: -1 }).limit(1);
    console.log("IN landingpage");
    res.render("landingPage", { user, product, producttwo, productThree });
  } catch (error) {
