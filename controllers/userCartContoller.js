...
userCartController.showCart = async (req, res) => {
  try {
    ...
    if (cart[0].products && cart[0].products.length > 0) {
      products = cart[0].products.filter(
        (product) => product.productId.isPublished
      );
...
userCartController.addProductToCart = async (req, res) => {
  try {
    ...
    const quantity = req.body.quantityValue
      ? parseInt(req.body.quantityValue, 10)
      : 1;
    ...
    try {
      ...
      if (product.stock < quantity) {
        return res
          .status(400)
          .send("Requested product is not available in stock");
      }
...
userCartController.removeProduct = async (req, res) => {
  ...
  try {
    ...
    const removedProduct = userCart.products.find(
      (product) => product.productId.toString() === productToRemove
    );
  ...
  try {
    ...
    if (userCart) {
      ...
      if (productInCart) {
        ...
        if (
          isNaN(quantityDifference) ||
          !isFinite(quantityDifference) ||
  ...
  try {
    ...
    cart.total = products.reduce((total, product) => {
      ...
      if (!isNaN(productTotal) && isFinite(productTotal)) {
        ...
        console.log(
          "Invalid product total for product",
