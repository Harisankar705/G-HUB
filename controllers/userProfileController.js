...
userProfileController.changePassword = async (req, res) => {
  try {
    ...
    if (currentPassword === "" || newPassword === "") {
      ...
      return res.json({
        status: "error",
        message: "Old and new password shouldn't be the same",
      });
    } else {
...
userProfileController.manageAddAddress = async (req, res) => {
  try {
    ...
    const { houseaddress, state, district, city, pincode, phonenumber } =
      req.body;
...
userProfileController.manageEditAddress = async (req, res) => {
  try {
    ...
    if (matchingAddresses.length > 0) {
      ...
      address.state = state;
      address.city = city;
      (address.pincode = pincode), (address.phonenumber = phonenumber);
    ...
    console.log(
      "An error occured while submitting edit address",
      error.message
    );
    res.render("error");
  }
...
userProfileController.customerCare = async (req, res) => {
  console.log("IN CUSTOMERCARE");
  ...
  try {
    ...
    const newCustomerCare = new customerCareSchema({
      userId: user._id,
      username: username,
      email: email,
      phonenumber: phonenumber,
    ...
    console.log("NEWCUSTOMERCARE", newCustomerCare);
    await newCustomerCare.save();
    return res.json({
      status: "success",
    ...
    return res
      .status(500)
      .json({ status: "error", message: "Failed to submit message" });
  }
};
